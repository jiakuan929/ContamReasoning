...
def per_sample_processing(
    ...
    for i, name in enumerate(module_names):
        ...
        # mi = compute_linear_cka(A_samples, G_samples).item()
        U_k = compute_Uk(A_samples, k).to(G_samples.dtype)

        mu = A_samples.mean(dim=0)
        A_c = A_samples - mu
        A_k = mu + A_c @ U_k @ U_k.T
        mi_alpha = gaussian_mi_gram_proper(A_k, G_samples)
        r = abs(mi - mi_alpha) / (mi + 1e-9)
        results.append(r)
        mi_ag.append(mi)
        mi_approx.append(mi_alpha)

    return results, mi_ag, mi_approx
...
if __name__ == "__main__":
    ...
    for sample in tqdm(unseen_data, desc="Unseen"):
        ...
        with torch.enable_grad():
            r, mi0, mi_approx = per_sample_processing(
                model, tok, question, target_response, args.k, module_names
            )
        unseen_results.append(r)
        unseen_mi.append(mi0)
        unseen_approx.append(mi_approx)
    unseen_results = np.array(unseen_results)
    unseen_mi = np.array(unseen_mi)
    unseen_approx = np.array(unseen_approx)
    ...
    for sample in tqdm(seen_data, desc="Seen"):
        ...
        seen_mi.append(mi0)
        seen_approx.append(mi_approx)
    seen_results = np.array(seen_results)
    seen_mi = np.array(seen_mi)
    seen_approx = np.array(seen_approx)

    this_time = datetime.now().strftime("%y-%m-%d-%H-%M")
    os.makedirs(args.output_dir, exist_ok=True)

    np.save(f"{args.output_dir}/unseen_error.npy", unseen_results)
    np.save(f"{args.output_dir}/seen_error.npy", seen_results)
    np.save(f"{args.output_dir}/unseen_mi.npy", unseen_mi)
    np.save(f"{args.output_dir}/seen_mi.npy", seen_mi)
    np.save(f"{args.output_dir}/unseen_approx.npy", unseen_approx)
    np.save(f"{args.output_dir}/seen_approx.npy", seen_approx)

    seen_results = seen_results.mean(axis=0)
    seen_mi = seen_mi.mean(axis=0)
    seen_approx = seen_approx.mean(axis=0)
    unseen_results = unseen_results.mean(axis=0)
    unseen_mi = unseen_mi.mean(axis=0)
    unseen_approx = unseen_approx.mean(axis=0)
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"{args.output_dir}/error.png")
    plt.close()

    plt.figure()
    plt.plot(unseen_mi, marker="o", markersize=4, label="Unseen MI")
    plt.plot(seen_mi, marker="^", markersize=4, label="Seen MI")
    plt.plot(unseen_approx, linestyle="--", label="Unseen Approx.")
    plt.plot(seen_approx, linestyle="--", label="Seen Approx.")
    plt.xlabel("Layer")
    plt.ylabel("MI")
    plt.title(f"{args.model_identifier}  {args.component}.{args.proj}_proj")
    plt.legend()
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"{args.output_dir}/mi.png")
    plt.close()
